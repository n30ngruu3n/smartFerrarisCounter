 ...
 *      check MQTT functionality (not sending?) - now sending.. 
 *          -> why ever MQTT must run in loop; maybe client.loop(); must be called with client.publish()..
...
void handleReset(){
  ...
  EEPROM.commit();
  EEPROM.end();
  Serial.println("Resetted values due to webhook and wrote to EEPROM");
  String s = "<a href='/'> Go Back </a>";
  server.send(200, "text/html", s);
}
...
void saveToEEPROMOnWebCommit() {
  ...
  EEPROM.commit();
  EEPROM.end();
  Serial.println("Saved repPerKWh and initialCounterValue to EEPROM");
  Serial.println("repPerKWh: " + String(repPerKWh));
  Serial.println("initialCounterValue: " + String(initialCounterValue));
}
...
void setup() {
  ...
  server.on("/resetValues", handleReset);
...
void loop() {
  ...
  // delay to not DDoS your MQTT Broker..
  if (millis() > previousMillisSend + intervalSend){
